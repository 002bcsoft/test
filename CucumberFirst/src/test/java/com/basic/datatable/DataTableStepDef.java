...
*/	 
	...
	public void enterFollowingData (DataTable table)  {
		List<List<String>> data = table.raw();
		
		/*for (int i=0; i<data.size(); i++) {
			for (int j=0; j<data.get(i).size(); j++)  {
				String vlaueFirstName = "";
				String valueLastName = "";
				if(j == 0)
		*/		
				String	vlaueFirstName = data.get(1).get(0);
				 	driver.findElement(By.xpath("//input[@id='u_0_l']")).sendKeys(vlaueFirstName);
		//		if (j == 1)
				String valueLastName = data.get(1).get(1);
				 	driver.findElement(By.xpath("//input[@id='u_0_n']")).sendKeys(valueLastName);
	/*		}
		}
	*/	
	}
